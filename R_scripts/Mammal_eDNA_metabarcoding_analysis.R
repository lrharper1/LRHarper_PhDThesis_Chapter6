...
p <- c("ggplot2","ggpubr","lemon","munsell","lazyeval","grid","gridExtra",
       "coda","MASS","car","scales","AICcmodavg","xtable",
...
cichlid.freq  <- cichlid/c(cichlid[116,])
...
## DNA contamination
max(cichlid.freq[65,222])   # 0.3077%
...
## all samples.
cichlid.test <- cichlid.test[-116,-222]
rownames(cichlid.test) <- NULL

total.counts <- data.frame(colSums(cichlid[-116,]))
...
cichlid.FP <- cichlid.conversion*c(cichlid.conversion[116,])

## Remove total row, reset row names, recreate Assignment column
cichlid.FP <- cichlid.FP[-116,]
...
## On average, 16.56% species detections are retained.
...
pos.freq <- pos.freq[,-1]
max(pos.freq[-c(65,114,116),17])   # 0.0644%
...
pos.FP <- pos.conversion*c(pos.conversion[116,])
...
## This would result in up to 96% taxa being removed.
## On average, 70% taxa are removed from samples.
...
SS.FP <- SS.conversion*c(SS.conversion[116,])
...
## On average, 7% taxa are removed from samples.
...
contaminants <- contaminants[,c(116,1:115)]
...
contaminants <- contaminants[,c(1,117,2:116)]
...
hm0 <- ggplot(contaminants, aes(x=ID, y=Assignment, fill=Frequency))
hm0 <- hm0 + geom_tile(colour="grey80")
hm0 <- hm0 + scale_fill_gradientn(name="Frequency\n", limits=c(0,1),
                                  colours = c("white","red", "black"), 
                                  values = c(0,0.1,1), labels=comma)
...
## Remove any assignments above species level
spp.df <- SS.FP[which(grepl(" ", SS.FP$Assignment)),]
...
## Plot detection across samples for each species by location
hm1 <- ggplot(cap.spp, aes(x=Sample_no, y=fct_rev(as_factor(Species))))
hm1 <- hm1 + geom_tile(aes(fill=Prop_reads), colour="black")
...
p2 <- p2 + geom_point(aes(x=preds.for.plot$Species, 
                      ...
                      position=position_dodge(width=0.5), cex=3)
...
p2 <- p2 + labs(x="Species", y="Proportional read counts")
p2 <- p2 + scale_y_continuous(limits=c(0,1),
                              breaks=c(0,0.25,0.50,0.75,1.0),
                              oob=squish)
p2 <- p2 + scale_shape_manual(name="Sample type",
                              values=c(16,17))
p2 <- p2 + scale_colour_manual(values=c("blue","chocolate4","cyan4",
...
## Plot the fitted data against the observed data
plot(cap.spp$Prop_reads ~ fitted(m2))

## Hosmer and Lemeshow Goodness of Fit Test
hoslem.test(cap.spp$Prop_reads, fitted(m2))
...
## source of heterogeneity i.e. independent variable that is non-linearly 
## associated with y
plot(sresid ~ cap.spp$Species_type)   
...
pdat <- expand.grid(Location = levels(cap.spp$Location),
                    ...
                    Species_type = levels(cap.spp$Species_type))
pdat

## Make a dataframe containing the predicted data
## re.form=NA means predictions are made for random effect as a whole
## se.fit=TRUE obtains the standard error for each of the predictions
predictions <- data.frame(predict(m2, newdata=pdat, se.fit=TRUE, 
...
preds.for.plot <- preds.for.plot[which(preds.for.plot$Location=="Wildwood Trust"),]
preds.for.plot <- preds.for.plot[which(preds.for.plot$Species=="Arvicola amphibius"),]
...
## Plot predicted data against observed data
p3 <- ggplot()
p3 <- p3 + geom_point(data=cap.spp,
                      aes(x=Species_type, y=Prop_reads, 
                          colour=Species, 
                          shape=Location,
                          group=Location), 
                      position=position_jitterdodge(dodge.width=0.3),
                      cex=2)
p3 <- p3 + geom_point(aes(x=preds.for.plot$Species_type, 
                          y=preds.for.plot$fit), 
                      position=position_dodge(width=0.5), cex=3)
p3 <- p3 + geom_errorbar(aes(x=preds.for.plot$Species_type, 
                             ymin=preds.for.plot$cil, 
                             ymax=preds.for.plot$ciu),
                         colour="black", size=0.8, width=0.3)
p3 <- p3 + labs(x="Species type", y="Proportional read counts")
p3 <- p3 + scale_y_continuous(limits=c(0,1),
                              breaks=c(0,0.25,0.50,0.75,1.0),
                              oob=squish)
p3 <- p3 + scale_colour_manual(guide=guide_legend(label.theme=element_text(face="italic", size=16)),
                               name="Species",
                               values=c("deepskyblue2","blue","chocolate4",
                                        "limegreen","cyan4","darkgreen",
                                        "purple","orange","hotpink"))
...
m3.df$ID <- droplevels(m3.df$ID)
m3.df$Enclosure <- droplevels(m3.df$Enclosure)
m3.df$Sample_no <- droplevels(m3.df$Sample_no)
m3.df$Species <- droplevels(m3.df$Species)
m3.df$Sample_type <- droplevels(m3.df$Sample_type)
m3.df$fSample_type <- droplevels(m3.df$fSample_type)
m3.df$Behaviour <- droplevels(m3.df$Behaviour)
...
## Plot boxplot against bubble plot of frequency of behaviours
p4 <- ggplot(m3.df, aes(x=Behaviour, y=Prop_reads))
p4 <- p4 + geom_jitter(aes(fill=Species, size=Frequency),
                       colour="black", pch=21, width=0.2, alpha=0.8)
p4 <- p4 + geom_boxplot(outlier.shape=NA, alpha=0.7)
p4 <- p4 + labs(x="Behaviour", y="Proportional read counts")
p4 <- p4 + scale_y_continuous(limits=c(0,1),
                              breaks=c(0,0.25,0.50,0.75,1.0))
p4 <- p4 + scale_fill_manual(name="Species",
                             values=c("blue","chocolate4","cyan4",
                                      "darkgreen","purple","orange"))
p4 <- p4 + guides(fill=guide_legend(override.aes=list(size=5),
...
p4a <- p4a + geom_jitter(aes(fill=Species, size=Frequency),
                         colour="black", pch=21, width=0.2, alpha=0.8)
...
p4a <- p4a + scale_y_continuous(limits=c(0,1),
                                breaks=c(0,0.25,0.50,0.75,1.0))
p4a <- p4a + scale_fill_manual(values=c("blue","chocolate4","cyan4",
                                        "darkgreen","purple","orange"))
...
p4a <- p4a + theme(panel.background = element_rect(fill = 'white'),
                   ...
                   legend.key=element_blank(),
                   legend.key.size = unit(2, 'lines'))
p4a <- p4a + facet_grid(. ~ Species, scale="free_x", space="free_x")
p4a
...
p4b <- p4b + geom_jitter(aes(fill=Species), colour="black",
                         pch=21, cex=3, width=0.2, alpha=0.8)
...
p5a <- p5a + geom_jitter(aes(x=cap.spp$Volume, y=cap.spp$Prop_reads, colour=cap.spp$Species), cex=2, width=0.2)
p5a <- p5a + geom_line(aes(x=coll.preds.for.plot1$Volume, y=coll.preds.for.plot1$fit))
p5a <- p5a + geom_ribbon(aes(x=coll.preds.for.plot1$Volume, ymin = coll.preds.for.plot1$cil, ymax = coll.preds.for.plot1$ciu), alpha = 0.25)
p5a <- p5a + labs(x="Volume (mL)", y="Reads")
p5a <- p5a + scale_y_continuous(limits=c(0,1), 
                                breaks=c(0,0.25,0.50,0.75,1.0))
p5a <- p5a + scale_colour_manual(guide=guide_legend(label.theme=element_text(face="italic", size=16)),
                                 name="Species",
                                 values=c("deepskyblue2","blue","chocolate4",
                                          "limegreen","cyan4","darkgreen",
                                          "purple","orange","hotpink"))
...
p5b <- p5b + geom_jitter(aes(x=cap.spp$No_filters, y=cap.spp$Prop_reads, colour=cap.spp$Species), cex=2, width=0.2)
p5b <- p5b + geom_line(aes(x=coll.preds.for.plot2$No_filters, y=coll.preds.for.plot2$fit))
p5b <- p5b + geom_ribbon(aes(x=coll.preds.for.plot2$No_filters, ymin = coll.preds.for.plot2$cil, ymax = coll.preds.for.plot2$ciu), alpha = 0.25)
p5b <- p5b + labs(x="Number of filters used", y="")
p5b <- p5b + scale_x_continuous(breaks=c(1,2))
p5b <- p5b + scale_y_continuous(limits=c(0,1), 
                                breaks=c(0,0.25,0.50,0.75,1.0))
p5b <- p5b + scale_colour_manual(guide=guide_legend(label.theme=element_text(face="italic", size=16)),
                                 name="Species",
                                 values=c("deepskyblue2","blue","chocolate4",
                                          "limegreen","cyan4","darkgreen",
                                          "purple","orange","hotpink"))
...
wild.pa <- wild.pa[c(4:5,9:11,23:26,31,34,36:37,40),]

## Reset row names for further indexing
rownames(wild.pa) <- NULL

## Create column specifying which lifestyle of each mammal species
wild.pa$Lifestyle <- ifelse(wild.pa$Assignment %in% wild.pa[c(1,4,7),1], "Semi-aquatic",
                            ifelse(wild.pa$Assignment %in% wild.pa[c(10,12),1], "Arboreal", "Ground-dwelling"))
...
## Rename columns
colnames(historic.dat)[3:4] <- c("Species","Historical data")
...
historic.dat$Lifestyle <- ifelse(historic.dat$Species %in% c("Arvicola amphibius","Castor fiber","Lutra lutra","Neovison vison"), "Semi-aquatic",
                                 ifelse(historic.dat$Species %in% c("Sciurus vulgaris"), "Arboreal", "Ground-dwelling"))
...
## Order species by new lifestyle factor
method.dat$fSpecies <- factor(method.dat$Species, 
                              levels=rev(unique(method.dat$Species)))
...
## Plot results
hm2 <- ggplot(method.dat, aes(x=fMethod, fSpecies))
hm2 <- hm2 + geom_tile(aes(fill=Occupancy, colour=Lifestyle), size=1)
hm2 <- hm2 + scale_colour_manual(values=c("goldenrod1","limegreen","skyblue2"))
hm2 <- hm2 + scale_fill_manual(name="",
...
hm2 <- hm2 + guides(fill=guide_legend(keywidth=0.5,
                    ...
                    colour=guide_legend(keywidth=0.5,
                                        keyheight=0.5,
                                        default.unit="inch"))
hm2 <- hm2 + facet_grid(Pond ~ Site, scales = "free", space = "free")
hm2
...
wild.final$fAssignment <- factor(wild.final$Assignment, 
                                 levels=rev(unique(wild.final$Assignment)))

## Plot proportional read counts
hm3 <- ggplot(wild.final, aes(x=Sample, y=fAssignment))
...
hm3 <- hm3 + theme(panel.grid.major = element_line(colour="white"),
                   ...
                   text = element_text(size=20),
                   legend.key.size = unit(1, 'lines'))
hm3 <- hm3 + facet_grid(Pond ~ Site, scales = "free", space = "free")
hm3
...
THL.exp$fAssignment <- factor(THL.exp$Assignment, 
                              levels=rev(unique(THL.exp$Assignment)))

## Plot results:
hm4 <- ggplot(THL.exp, aes(x=Sample, y=fAssignment, fill=Prop_reads))
